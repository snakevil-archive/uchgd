...
all: build/cmds-chk.log build/authorized_keys build/hgrc build/sshd_config \
		build/usermod.sh
	$(warning Runs '$@'...)
...
# {{{ 自定义目标：build/authorized_keys

build/authorized_keys:: $(wildcard pubkeys/*.pub)
	$(warning Generates '$@'...)
	$(if $^, , $(error One public key required at least))
	'mkdir' -p $(@D)
